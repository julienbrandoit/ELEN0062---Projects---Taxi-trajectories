...
for L_PADDING in param_grid:
    ...
    def process(S):
        ...
        if first_time:
            ...
            category_frequencies_stand = s['ORIGIN_STAND'].value_counts(normalize=True)
            ...
            top_categories_stand = category_frequencies_stand.nlargest(255).index
        ...
        s['ORIGIN_CALL'] = s['ORIGIN_CALL'].apply(lambda x : category_mapping_origin_call.get(x, -1))
        s['ORIGIN_STAND'] = s['ORIGIN_STAND'].apply(lambda x : category_mapping_origin_stand.get(x, -1))
        s['TAXI_ID'] = s['TAXI_ID'].apply(lambda x : category_mapping_taxi_ID.get(x, -1))
        """
    ...
    lat_estimator = RandomForestRegressor(n_jobs=6, n_estimators=500, bootstrap=True, verbose=1, max_features='sqrt')
    long_estimator = RandomForestRegressor(n_jobs=6, n_estimators=500, bootstrap=True, verbose=1, max_features='sqrt')
    ...
    score_L[L_PADDING] = np.mean(score)
    
    import matplotlib.pyplot as plt
    plt.hist(score, bins=100, range=(0,10))
