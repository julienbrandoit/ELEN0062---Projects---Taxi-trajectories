...
def process_set(dataset, size, test_set = False, validation=False):
    ...
    s['x'] = s.apply(lambda row: row['POLYLINE'][max(0, row['r']-L_PADDING):row['r']], axis=1)
    ...
    #s.drop(['r'], inplace=True, axis=1)
    s['x1'] = s['x'].apply(lambda x: [p[0] for p in x])
    s['x2'] = s['x'].apply(lambda x: [p[1] for p in x])

    s['x1'] = s['x1'].apply(lambda x: np.pad(x, (0,L_PADDING - len(x))))
    s['x2'] = s['x2'].apply(lambda x: np.pad(x, (0,L_PADDING - len(x))))
    ...
    #s['DAY_TYPE_A'] = s['DAY_TYPE'].apply(lambda x : 1 if x == 'A' else 0)
    #s['DAY_TYPE_B'] = s['DAY_TYPE'].apply(lambda x : 1 if x == 'B' else 0)
    #s['DAY_TYPE_C'] = s['DAY_TYPE'].apply(lambda x : 1 if x == 'C' else 0)
...
#catego_features = ['CALL_TYPE_A', 'CALL_TYPE_B', 'CALL_TYPE_C', 'DAY_TYPE_A', 'DAY_TYPE_B', 'DAY_TYPE_C', 'ORIGIN_CALL', 'ORIGIN_STAND', 'TAXI_ID', 'month', 'day']
...
def init():
    ...
    data_VS = data.drop(data_LS.index)
    ...
    print("-- Normalizing data : LS", flush = True)
    #X_train = normalize_X(X_train, test = False)
    print("-- Normalizing data : VS", flush = True)
    #X_val = normalize_X(X_val, test=True)
    print("-- Normalizing data : TS", flush = True)
    #TS = normalize_X(TS, test=True)
    """
